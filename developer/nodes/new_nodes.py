...
def get_new_nodes(prompt, nodes, table_completion_gpt,
                  headers=["type", "file_path", "file_extension", "name", "inputs",
                           "outputs", "is parent", "parent class", "short description"],
                  verbose_headers=["type", "file_path(required, make it descriptive)", "file_extension(required)", "name(required)", "inputs(leave blank if class nodes)", "outputs(leave blank if class nodes)", "is parent[one of True,False]", "parent class(the class from which it inherits)", "short description"],
    ...
    try:
        ...
        for raw_new_node in raw_new_nodes:
            ...
            if file_extension and ('None' not in file_extension):
                ...
                file_extension = ''
            file_name = new_node.pop('file_path')
