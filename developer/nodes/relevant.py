...
def get_relevant_nodes(prompt, selection_gpt, table_completion_gpt=None, relevant_files=[], nodes_by_file=None, minimum_node_relevance_rating=2):
    ...
    GET_RELEVANT_NODES_PROMPT_FORMAT = """find the relevant resources (variables, functions, function call, classes, ...) that contain can be reused would need to be modified or linked for the task: {task}"""
    ...
    for file_name, file_nodes in nodes_by_file:
        ...
        logger.debug(f"raw_relevant_nodes {file_name}: {raw_relevant_nodes}")
